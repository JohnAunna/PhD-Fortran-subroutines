...
c------------------------------------------------calculating the convection term for all cells
      conv_x(ic)=den(ic)*(u(1,ic)*g1(1,ic)+u(2,ic)*g1(2,ic)
     X +u(3,ic)*g1(3,ic))
	conv_y(ic)=den(ic)*(u(1,ic)*g2(1,ic)+u(2,ic)*g2(2,ic)
     X  +u(3,ic)*g2(3,ic))
	conv_z(ic)=den(ic)*(u(1,ic)*g3(1,ic)+u(2,ic)*g3(2,ic)
     X +u(3,ic)*g3(3,ic))
	    End do
c---------------------------------------------------------------------------
...
c------------------------------------------------calculating the pressure term for all cells
      ...
      p_z(ic)=g1(3,ic)
        ...
        del2w(ic)=conv_z(ic)+p_z(ic)+9.81*den(ic)  !.+.-Gravity
c---------------------------------------------------------------------non-dimensionalized (magnitude of viscous term/ gravity term)
        del2(ic)=sqrt((del2u(ic))**2+(del2v(ic))**2+(del2w(ic))**2) 
     X              /(9.81*den(ic)) ! rho.g
		 END DO
!		 write(6,*)'terms value: convection, gravity, pressure, viscous'
!		 write(6,*) conv_z(5712),9.81*den(5712),p_z(5712),del2w(5712)
